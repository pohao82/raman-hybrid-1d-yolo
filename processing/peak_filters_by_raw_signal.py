...
def _find_quiet_region_sigma(signal, detections, W, quiet_window=71, eps=1e-8):
    """
    Find the widest gap between two adjacent detected peaks, take a
    quiet_window-sample region centered in that gap, use its spread as the
    noise sigma for the whole spectrum.
    """
    if len(detections) < 2:
        return max(np.std(signal), eps), (0, len(signal))

    positions = sorted(d[2] for d in detections)
    gaps = [(positions[i+1] - positions[i], positions[i], positions[i+1])
            for i in range(len(positions) - 1)]
    _, p_lo, p_hi = max(gaps, key=lambda g: g[0])
    mid_pos = 0.5 * (p_lo + p_hi)

    # Mapping the position to array indices bounded between [0, len(signal)-1]
    idx = np.clip(np.searchsorted(W, mid_pos), 0, len(signal) - 1)
    half = quiet_window // 2
    lo, hi = max(0, idx - half), min(len(signal), idx + half + 1)

    region = abs(signal[lo:hi])
    sigma = max(np.std(region), eps)
    #sigma = np.mean(region)
    return sigma, (lo, hi)


def compute_multiscale_diff_movavg(signal, detections, window_bank, W,
                                   span=15, polyorder=1,
                                   quiet_window=101, eps=1e-8):
    signal = np.asarray(signal, dtype=float)

    sigma, quiet_range = _find_quiet_region_sigma(signal, detections, W,
                                                    quiet_window=quiet_window, eps=eps)
    ...
    for conf, A, pos, gamma in detections:
        ...
        for w_short, w_long in zip(window_bank[:-1], window_bank[1:]):
            diff = smooths[w_short] - smooths[w_long]
            i_diff = (diff / sigma) ** 2
            i_diff = savgol_filter(i_diff, window_length=7, polyorder=polyorder)
            vals.append(i_diff[lo:hi].max())
        profiles.append(np.array(vals))
    return profiles, sigma, quiet_range


def filter_by_multiscale_movavg(detections, W, raw_signal, cfg, threshold,
                                window_bank=(11, 35, 71), span=30,
                                polyorder=1, quiet_window=101):
    """Keep each detection whose noise-normalized multiscale i_diff exceeds
    `threshold` at ANY scale-pair. i_diff = ((smooth_short - smooth_long)/sigma)**2,
    so `threshold` is roughly (n-sigma)**2 (e.g. 9 == 3-sigma).

    Pass the UNSMOOTHED resampled signal -- the noise-sigma estimate depends on it.
    `cfg` is unused (kept for a uniform filter signature with filter_by_raw_signal_flat).

    In:  detections -- list of (conf, A, pos, gamma)
    Out: list of (conf, A, pos, gamma) -- the score is preserved (like _flat).
    """
    if len(detections) == 0:
        return []
    profiles, _sigma, _quiet_range = compute_multiscale_diff_movavg(
        raw_signal, detections, list(window_bank), W,
        span=span, polyorder=polyorder, quiet_window=quiet_window,
    )
    return [d for d, profile in zip(detections, profiles) if np.any(profile > threshold)]
